...
const drawerWidth = useCookie<number | string>('drawerWidth', { sameSite: true });
...
const loadDrawer = async () => {
    ...
    drawer.value.data = (await $fetch<apiResponse>(`${appConfig.apiBase}${database.value.slug}/${drawer.value.table}/${drawer.value.id}`)).result;
...
watch(drawer, (value, originalValue) => {
    if (value?.show && value.id && (!Object.keys(value.data).length || value.table !== originalValue.table || value.id !== originalValue.id)) loadDrawer();
});

const toggleDrawerWidth = () => {
    if (typeof drawerWidth.value === 'string')
        drawerWidth.value = 251
    else
        drawerWidth.value = drawerWidth.value >= screenHalf ? 251 : '100%';
};

const filteredSchema = computed(() => database.value.tables
    ?.find(({ slug }) => slug === drawer.value.table)
    ?.schema?.filter(({ key }) => !['id', 'createdAt', 'createdBy', 'updatedAt'].includes(key)) ?? []);
</script>