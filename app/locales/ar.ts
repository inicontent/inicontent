export default {
	ar: {
		// Actions
		edit: "تعديل",
		create: "إنشاء",
		cancel: "إلغاء",
		save: "حفظ",
		submit: "إرسال",
		update: "تحديث",
		search: "بحث",
		filter: "تصفية",
		reset: "إعادة تعيين",
		close: "إغلاق",
		confirm: "تأكيد",
		add: "إضافة",
		remove: "إزالة",
		upload: "رفع",
		download: "تنزيل",
		export: "تصدير",
		import: "إستيراد",
		refresh: "تحديث",
		set: "تعيين",
		unset: "إفراغ",
		replace: "إستبدال",
		rename: "تغيير الإسم",
		new: "جديد",
		flows: "تدفقات",
		actions: "أوامر",
		createFromClipboard: "إنشاء من الحافظة",
		view: "عرض",
		rightClickToEdit: "إضغط يمين للتعديل",
		pressAndHoldToEdit: "إضغط مطولاً للتعديل",
		copyItem: "نسخ العنصر",
		pasteItem: "لصق العنصر",
		copyFields: "نسخ الحقول",
		pasteFields: "لصق الحقول من الحافظة",
		noFieldsInClipboard: "لا توجد حقول في الحافظة",
		noFieldsToCopy: "لا توجد حقول للنسخ",
		cutSuccessfully: "تم القص بنجاح",
		clipboardItemIsNotCorrect: "عنصر الحافظة غير صحيح",
		publish: "نشر",
		selectAll: "تحديد الكل",
		clearAll: "إلغاء تحديد الكل",

		// Status messages
		success: "نجح",
		error: "خطأ",
		warning: "تحذير",
		info: "معلومة",
		loading: "جاري التحميل",

		// Common nouns
		name: "الإسم",
		slug: "المعرّف النصّي",
		description: "الوصف",
		title: "العنوان",
		content: "المحتوى",
		type: "النوع",
		status: "الحالة",
		date: "التاريخ",
		time: "الوقت",
		size: "الحجم",
		link: "الرابط",
		label: "النص",
		icon: "الأيقونة",
		username: "إسم المستخدم",
		email: "البريد الإلكتروني",
		password: "كلمة المرور",
		role: "الصلاحية",
		item: "عنصر",
		anItem: "عنصر",
		newItem: "عنصر جديد",
		asFollow: "كالآتي",
		latestActivities: "أخر العمليات",
		revertedToVersion: "تم الاستعادة للإصدار",
		revertToVersion: "استعادة للإصدار",
		revertConfirmMessage:
			"سيتم استعادة العنصر لحالته في الوقت المحدد. هل أنت متأكد؟",
		revert: "استعادة",
		revertSuccess: "تم استعادة العنصر بنجاح",
		revertFailed: "فشل في استعادة العنصر",

		// Navigation
		home: "الرئيسية",
		back: "رجوع",
		next: "التالي",
		previous: "السابق",

		// Validation messages
		isRequired: "إجباري",
		required: "إلزامي",
		optional: "اختياري",
		isInvalidFormat: "صيغة غير صحيحة",
		isNotValid: "غير صالح",
		inputsAreInvalid: "المُدخلات غير صحيحة",

		// Confirmation messages
		theFollowingActionIsIrreversible: "الإجراء التالي لا رجعة فيه",
		areYouSure: "هل أنت متأكد؟",
		deleteSelectedAssetsConfirm:
			"حذف {count} عنصر محدد؟ لا يمكن التراجع عن هذا الإجراء.",
		deleteAssetConfirm: "حذف هذا الملف؟ لا يمكن التراجع عن هذا الإجراء.",
		deleteFolderConfirm:
			'حذف مجلد "{name}" وكل ما بداخله؟ لا يمكن التراجع عن هذا الإجراء.',
		deletedSuccessfully: "تم الحذف بنجاح",
		savedSuccessfully: "تم الحفظ بنجاح",
		updatedSuccessfully: "تم التحديث بنجاح",
		createdSuccessfully: "تم الإنشاء بنجاح",
		uploadFailed: "فشل الرفع",
		folderCreatedSuccessfully: "تم إنشاء المجلد بنجاح",
		folderNameRequired: "إسم المجلد مطلوب",
		filterSavedSuccessfully: "تم حفظ المرشح بنجاح",
		filterDeletedSuccessfully: "تم حذف المرشح بنجاح",
		filterLoaded: "تم تحميل المرشح",

		// Methods
		get: "عرض",
		post: "إنشاء",
		put: "تعديل",
		delete: "حذف",
		deleteSelectedItems: "حذف العناصر المحددة",

		// API
		apiDocumentation: "دليل الواجهة البرمجية",
		apiDocs: {
			availableMethods: "العمليات المتاحة",
			sessionNoticeTitle: "اعتماد الجلسة",
			sessionNoticeDescription:
				"يمكنك تمرير معرف الجلسة عبر الوسيط {param} سواءً كمعلَمة استعلام أو كملف تعريف ارتباط.",
			sessionAsQuery: "كمعلَمة في رابط الطلب:",
			sessionAsCookie: "أو كملف تعريف ارتباط يُرسل مع كل طلب.",
			sessionAuthLinkPrefix: "للحصول على معرف جلسة، اطّلع على",
			viewAuthDocs: "إرشادات المصادقة",
			schemaTitle: "حقول المخطط",
			assetsUploadTitle: "إرشادات رفع الملفات",
			assetsUploadIntro:
				"أرسل طلب POST إلى ‎/assets أو ‎/assets/<folder> متضمناً كائناً أو مصفوفة كائنات تصف الملفات المطلوب رفعها.",
			assetsUploadStep1:
				"يجب أن يحتوي كل كائن على الحقول name و size و type و extension، ويمكن إرسال مصفوفة عند رفع عدة ملفات.",
			assetsUploadStep2:
				"يعيد الرد نفس الحقول بالإضافة إلى id و createdAt و publicURL و uploadURL.",
			assetsUploadStep3:
				"بعد ذلك أرسل الملف الثنائي إلى عنوان uploadURL عبر طلب POST.",
			assetsUploadCustomEndpoint:
				"إذا استخدمت نقطة رفع مخصّصة فأرفق publicURL في أول طلب إلى ‎/assets، ولن يتم إرجاع حقل uploadURL.",
			noSchemaFields: "لا توجد حقول معرفة لهذا الجدول.",
			tableNotFoundTitle: "لم يتم العثور على الجدول",
			tableNotFoundDescription: "تحقق من المعرّف أو اختر جدولاً آخر من القائمة.",
			backToTables: "العودة إلى قوائم الواجهة البرمجية",
			realtime: {
				enabled: "المزامنة الفورية مفعلة",
				title: "التحديثات الفورية",
				description:
					"يبث هذا الجدول تغييرات البيانات مباشرة عبر اتصال WebSocket. اتصل بالنقطة الطرفية أدناه واشترك لتلقي أحداث الإنشاء والتحديث والحذف لحظة حدوثها.",
				endpointLabel: "نقطة اتصال WebSocket",
				subscribeLabel: "رسالة الاشتراك",
				subscribeHint:
					"أرسل هذه الرسالة فور فتح الاتصال. {database} و {table} هما معرفا قاعدة البيانات والجدول، و {param} يحمل معرف جلستك.",
				eventsLabel: "الأحداث الواردة",
				eventsHint:
					"يرسل الخادم رسائل data_change بإجراء إنشاء أو تحديث أو حذف، ويُعاد الاتصال تلقائياً عند انقطاعه.",
			},
			auth: {
				title: "نهايات المصادقة",
				description:
					"استخدم هذه النهايات لإنشاء الجلسات وتمرير الوسيط {param}.",
				signinDescription:
					"يصدق مستخدماً موجوداً؛ تنشئ POST جلسة جديدة بينما تحدّث PUT الجلسة الحالية.",
				usernameField: "username — اسم المستخدم الفريد",
				passwordField: "password — كلمة المرور (يتم تشفيرها على الخادم)",
				signupDescription:
					"ينشئ مستخدماً جديداً وفق مخطط جدول users الموضح أدناه.",
				currentDescription: "يعيد بيانات المستخدم المرتبطة بالرمز {param}.",
				signoutDescription:
					"ينهي الجلسة النشطة ويمسح ملف تعريف الارتباط المتعلق بها.",
				signupSchemaTitle: "مخطط جدول المستخدمين",
				cookieHint:
					"بعد تسجيل الدخول يُخزَّن معرف الجلسة في ملف تعريف الارتباط {param}، ويمكنك أيضاً تمريره يدوياً عند الحاجة.",
			},
		},

		// Units
		units: {
			kB: "ك.ب",
			MB: "م.ب",
			GB: "ج.ب",
			TB: "ت.ب",
		},

		// Plural examples
		items: {
			zero: "لا توجد عناصر",
			one: "عنصر واحد",
			two: "عنصران",
			few: "{{count}} عناصر",
			many: "{{count}} عنصر",
			other: "{{count}} عنصر",
		},

		compression: {
			label: "ضغط البيانات",
			description:
				"يقلل ضغط البيانات من المساحة المستخدمة ولكنه قد يُبطئ النتائج",
			videoLarge:
				"تم اكتشاف فيديو كبير ({size}). سنقوم بتحسينه محلياً قبل الرفع.",
			videoHuge:
				"تم اكتشاف فيديو ضخم ({size}). تتم عملية الضغط محلياً وقد تستغرق بعض الوقت.",
			pdfLarge:
				"تم اكتشاف ملف PDF كبير ({size}). سنعيد بناءه في متصفحك قبل الرفع.",
			pdfHuge:
				"تم اكتشاف ملف PDF ضخم ({size}). قد يستغرق عرض الصفحات محلياً بعض الوقت.",
		},

		// Dashboard
		settings: "الإعدادات",
		toggleTheme: "تغيير الوضع",
		adminPanel: "لوحة التحكم",
		allRightsReserved: "جميع الحقوق محفوظة",
		logout: "تسجيل الخروج",
		profile: "تعديل الحساب",
		sessions: "سِجل الدخول",
		blocks: "أجزاء الصفحات",
		pages: "الصفحات",
		users: "المستخدمين",
		admin: "مدير",
		user: "مستخدم",
		guest: "زائر",
		translations: "الترجمة",
		assets: "الملفات",
		databases: "قواعد البيانات",
		createDatabase: "إنشاء قاعدة بيانات",
		basicInformation: "المعلومات الأساسية",
		basicInformationDescription: "حدد تفاصيل قاعدة البيانات واللغة الافتراضية.",
		buildTables: "إنشاء الجداول",
		buildTablesDescription: "صف الجداول المطلوبة وسننشئ لك مخططاً أولياً.",
		tables: "الجداول",
		table: "جدول",
		totalDatabaseSize: "حجم قاعدة البيانات",
		showAll: "أظهر الكل",
		roles: "الأدوار",
		primaryLanguage: "اللغة الأساسية",
		secondaryLanguages: "اللغات الثانوية",
		languages: {
			ar: "العربية",
			en: "الإنجليزية",
			fr: "الفرنسية",
			es: "الإسبانية",
		},
		tableSettings: "إعدادات الجدول",
		primaryColor: "اللون الأساسي",
		primaryDarkColor: "اللون الأساسي في الوضع الليلي",
		auth: "تسجيل الدخول",
		createdAt: "تاريخ الإنشاء",
		updatedAt: "تاريخ التحديث",
		createdBy: "أُنشأ من قبل",
		id: "المعرف",
		copiedSuccessfully: "تم النسخ بنجاح",
		skipCompression: "تخطي الضغط",
		print: "طباعة",

		// Auth
		signin: "تسجيل الدخول",
		signup: "إنشاء حساب",
		authentication: "صفحة الدخول",
		forgotPassword: "هل نسيت كلمة المرور؟",
		sendResetLink: "إرسال رابط إعادة التعيين",
		resetPassword: "إعادة تعيين كلمة المرور",
		newPassword: "كلمة المرور الجديدة",
		confirmPassword: "تأكيد كلمة المرور",
		passwordMinimumLength: "يجب ألا تقل كلمة المرور عن 8 أحرف.",
		passwordsDoNotMatch: "كلمتا المرور غير متطابقتين.",
		invalidResetLink:
			"رابط إعادة تعيين كلمة المرور غير صالح أو منتهي الصلاحية.",
		backToSignIn: "العودة إلى تسجيل الدخول",

		// Table actions
		tools: "الأدوات",
		clearTable: "إفراغ الجدول",
		exportCurrentData: "تصدير البيانات الحالية",
		exportAllData: "تصدير كل البيانات",
		databaseExport: "تصدير قاعدة البيانات",
		databaseExportDescription:
			"إنشاء أرشيف خاص لجميع المخططات والسجلات. ملفات الأصول غير مضمنة.",
		startDatabaseExport: "إنشاء التصدير",
		databaseExportFailed: "فشل تصدير قاعدة البيانات",
		databaseExportExpires: "متاح حتى {date}",
		databaseExportState: {
			queued: "تم وضع التصدير في قائمة الانتظار",
			running: "جاري تصدير قاعدة البيانات",
			completed: "التصدير جاهز",
			failed: "فشل التصدير",
			expired: "انتهت صلاحية التصدير",
		},
		columns: "الأعمدة",
		an_export_job_is_running_in_background: "هناك عملية تصدير جارية",
		an_export_job_is_done: "عملية التصدير إنتهت",
		uploadingImportFile: "جاري رفع ملف الاستيراد",
		an_import_job_is_running_in_background: "هناك عملية استيراد جارية",
		an_import_job_is_done: "اكتملت عملية الاستيراد",
		an_import_job_failed: "فشلت عملية الاستيراد",
		importedRows: "تم استيراد {count} عنصر",
		kanban: "كانبان",
		small: "صغير",
		medium: "متوسط",
		large: "كبير",
		newTable: "جدول جديد",
		newItem: "عنصر جديد",
		schedules: "الجدولات",
		tableSchedules: "جدولات الجدول",
		runNow: "تشغيل الآن",
		runAllActiveSchedules: "تشغيل جميع الجدولات النشطة",
		noActiveSchedules: "لا توجد جدولات نشطة للتشغيل",
		allActiveSchedulesRan: "تم تشغيل جميع الجدولات النشطة بنجاح",
		someSchedulesFailed: "فشل تشغيل {count} جدولة",
		tableSlug: "إسم الجدول",
		quickSettings: "إعدادات سريعة",
		makeItPrivate: "اجعله خاصًا",
		read: "قراءة",
		clickToToggleDate: "إضغط لتغيير التاريخ",

		// Table settings
		generalSettings: "إعدادات عامة",
		schemaSettings: "إعدادات الأعمدة",
		changeOrder: "تغيير الترتيب",
		deleteTable: "حذف الجدول",
		defaultSearchableColumns: "الأعمدة الإفتراضية للبحث",
		disableIdEncryption: "إلغاء تشفير المعرف, وإعتماد الأرقام",
		decodeID: "إلغاء تشفير المعرف",
		prepend: "إضافة مقدمة",
		recentItemsAppearAtTheTop: "العناصر الحديثة تظهر في الأعلى",
		cache: "التخزين المؤقت",
		cacheDescription:
			"يُظهر التخزين المؤقت النتائج بشكل أسرع ولكنه قد يزيد من حجم قاعدة البيانات",
		clearCache: "حذف الكاش",
		log: "سجل التتبع",
		enableActivityLog: "تفعيل سجل عمليات الجدول",
		realtime: "المزامنة الفورية",
		enableRealtimeSync: "تفعيل المزامنة الفورية للبيانات",
		realtimeEnabled: "المزامنة الفورية مفعلة",
		connectingToRealtime: "جاري الاتصال بالمزامنة الفورية...",
		defaultTableColumns: "الأعمدة الإفتراضية للجدول",
		displayAs: "إظهار كـ",
		tableItemLabel: "تسمية عناصر الجدول",
		cards: "بطاقات",
		list: "قائمة",
		urlSuffix: "لاحقة الرابط",
		config: "الإعدادات",

		// Schema fields
		fieldName: "إسم الحقل",
		fieldDescription: "وصف الحقل",
		allowedFiles: "الملفات المسموح بها",
		options: "الخيارات",
		valuesType: "نوع القيم",
		tableName: "إسم الجدول",
		expandByDefault: "توسيع بشكل افتراضي",
		dragToMove: "اسحب للتحريك",
		fileType: {
			image: "صورة",
			video: "فيديو",
			audio: "صوت",
			document: "نص",
			archive: "أرشيف",
		},
		allowCustomValues: "السماح بإدخال قيم جديدة",
		unique: "فريد",
		uniqueGroup: "مجموعة فريدة",
		regex: "التعبير النمطي",
		computedExpression: "تعبير محسوب",
		computedExpressionPlaceholder: "مثال: sum(5 * 6)",
		computedExpressionDocs: {
			whatIsIt: "ما هو؟",
			whatIsItBody:
				"حقل محسوب يعيد المحرك حسابه تلقائيًا في كل مرة يُحفظ فيها السجل. النتيجة تُخزَّن في السجل وتكون للقراءة فقط — لا يمكن تعديلها يدويًا ولا إرسالها عند إنشاء السجلات أو تحديثها. البادئة واللاحقة تغيّران طريقة العرض فقط.",
			example: "مثال عملي — إجمالي الطلب",
			exampleBody:
				"إذا كان الجدول يحتوي مصفوفة items فيها quantity (معرف 5) و price (معرف 6)، فهذا الحقل يحسب الإجمالي لكل العناصر:\n`total = sum(5 * 6)`\n`items: [{quantity: 2, price: 250}, {quantity: 1, price: 100}]`\n`النتيجة = (2 × 250) + (1 × 100) = 600`",
			referencing: "الإشارة إلى الحقول",
			referencingBody:
				"كل حقل يُشار إليه بالرقم الظاهر بين قوسين تحت اسمه — مثلًا items.quantity (5) تعني الحقل رقم 5، أي صف quantity داخل items.\nالرقم الذي يطابق معرّف أحد الحقول يمثل هذا الحقل مباشرة؛ أما الرقم الذي لا يطابق أي حقل فيُعامل كرقم عادي. فمثلًا `6 / 4` يقسم الحقل 6 على الحقل 4، بينما `314 / 100` يقسم على 100 (لا يوجد حقل رقمه 100).",
			operators: "العوامل",
			operatorsBody:
				"`+` جمع، `-` طرح، `*` ضرب، `/` قسمة، `%` باقي القسمة، و`(...)` للتجميع.\nالعوامل `* / %` أقوى من `+ -`: `2 * 3 + 4` تساوي 10، بينما `2 * (3 + 4)` تساوي 14.",
			functions: "دوال المصفوفات",
			functionsBody:
				"الدوال sum و count و avg و min و max تعمل على عناصر حقل من نوع «مصفوفة من الكائنات». الوسيط تعبير كامل يُحسب لكل عنصر على حدة ثم تُجمَّع النتائج:\n`sum(5 * 6)` = مجموع (الكمية × السعر) لكل عنصر؛ `count(5)` = عدد العناصر؛ `avg(5)` = المتوسط؛ `min(5)` / `max(5)` = الأدنى / الأعلى.",
			decimals: "الكسور العشرية",
			decimalsBody:
				"لا توجد أعداد عشرية في التعبير — لأن `.` محجوز للروابط، فمثلًا `3.14` مسار وليس 3.14. النتائج الكسرية مقبولة تمامًا: `314 / 100` يعطي 3.14.",
			tables: "الجداول المرتبطة",
			tablesBody:
				"يمكنك الانتقال إلى جدول مرتبط باستخدام `.`: `3.4` يقرأ الحقل 4 من السجل المرتبط بالحقل 3. ويُسمح بذلك داخل الدوال أيضًا: `sum(5 * 4.2)` = (الكمية × سعر المنتج المرتبط).",
			children: "الحقول المحسوبة في العناصر الفرعية",
			childrenBody:
				"الحقل المحسوب على عنصر فرعي داخل مصفوفة كائنات يُعاد حسابه لكل عنصر على حدة. يمكنك الإشارة إلى حقول العنصر نفسه بالمعرّفات، والانتقال إلى الجداول المرتبطة بـ `.`: بافتراض أن items.quantity المعرف 4 و items.product المعرف 3 (وسجل product مرتبط بحقل price معرفه 2)، فإن `lineTotal = 4 * 3.2` يحسب (الكمية × سعر المنتج المرتبط) لكل عنصر.\nالمعامل المفقود أو الفارغ يكتب 0 بدلًا من الفشل. قيمة العنصر الفرعي للقراءة فقط — إرسالها عند الحفظ مرفوض — ويجب أن يكون نوع الحقل الفرعي number.\nفي عداد لوحة المعلومات، اختر الحقل بالنقاط (مثل items.lineTotal) مع عملية الجمع.",
			notes: "ملاحظات",
			notesBody:
				"لا يمكن أن يكون الحقل محسوبًا ومطلوبًا/فريدًا/بنمط regex في نفس الوقت — تُمسح هذه الخصائص تلقائيًا عند تعيين تعبير. تُعاد معالجة الحقول المحسوبة بترتيب الاعتماديات، لذا يمكن لحقل محسوب أن يستخدم حقلًا محسوبًا آخر. أي تعبير غير صالح يُرفض عند حفظ المخطط.",
		},
		computedPrefix: "بادئة",
		computedSuffix: "لاحقة",
		computedAffixPlaceholder: "للعرض فقط — مثل $ أو %",
		uploadParams: "إعدادات الرفع",
		minimumItems: "الحد الأدنى للعناصر",
		maximumItems: "الحد الأقصى للعناصر",
		ie: "على سبيل المثال",
		useInison: "إستعمل Inison",
		extendWhere: "توسيع البحث",
		labelsColoring: "تفعيل الألوان",
		optionLabel: "إسم الخيار",
		optionColor: "لون الخيار",
		optimizeAssets: "تحسين الملفات",
		optimizeAssetsDescription: "ضغط وتحسين الملفات المرفوعة تلقائياً لأداء أفضل",
		fieldTypeNotExisting: "نوع الحقل غير موجود",

		// Activity log
		latestActivities: "أخر العمليات",
		updated: "قام بتعديل",
		deleted: "قام بحذف",
		removed: "محذوف",
		created: "قام بإنشاء",
		asFollow: "كالأتي",
		in: "في",
		to: "إلى",
		from: "من",
		as: "كـ",

		// Assets
		uploadFiles: "رفع ملفات",
		uploadFolder: "رفع مجلد",
		createFolder: "إنشاء مجلد",
		dropFilesHere: "أسقط الملفات هنا",
		folderName: "إسم المجلد",
		image: "صورة",
		video: "فيديو",
		gallery: "قائمة الملفات",
		assetLink: "رابط الملف",

		// Search / Filters
		convert_to_and_group: "تحويل إلى مجموعة و",
		convert_to_or_group: "تحويل إلى مجموعة أو",
		andGroup: "مجموعة و",
		orGroup: "مجموعة أو",
		favoriteFilters: "المرشحات المفضلة",
		filterName: "اسم المرشح",
		filterNameCannotBeEmpty: "اسم المرشح لا يمكن أن يكون فارغاً",
		filterNameAlreadyExists: "اسم المرشح موجود بالفعل",
		noFavoriteFilters: "لا توجد مرشحات مفضلة محفوظة",
		relativeGroup: "نسبي",
		relativePlaceholder: "مثال: قبل 3 أيام",
		day: "يوم",
		days: "أيام",
		week: "أسبوع",
		weeks: "أسابيع",
		month: "شهر",
		months: "شهور",
		year: "سنة",
		years: "سنوات",
		ago: "مضت",
		now: "الآن",
		yesterday: "أمس",
		tomorrow: "غدًا",
		"last week": "الأسبوع الماضي",
		"last month": "الشهر الماضي",
		"last year": "السنة الماضية",
		"next week": "الأسبوع القادم",
		"next month": "الشهر القادم",
		"next year": "السنة القادمة",

		// Comparison operators
		ComparisonOperator: {
			equalTo: "يساوي",
			notEqualTo: "لا يساوي",
			greaterThan: "أكبر من",
			greaterOrEqualTo: "أكبر من او يساوي",
			lessThan: "أصغر من",
			lessOrEqualTo: "أصغر من او يساوي",
			contains: "يحتوي",
			doesNotContain: "لا يحتوي",
			isOneOf: "يضم",
			isNotOneOf: "لا يضم",
		},

		// Field types
		fields: {
			string: "نص",
			shortText: "نص قصير",
			longText: "نص طويل",
			html: "محرر نصوص",
			number: "رقم",
			computed: "رقم محسوب",
			password: "كلمة مرور",
			link: "رابط",
			email: "بريد",
			color: "لون",
			asset: "ملحق",
			assets: "ملحقات",
			tags: "وسوم",
			date: "تاريخ",
			dateTime: "تاريخ ووقت",
			dateRange: "نطاق تاريخ",
			dateTimeRange: "نطاق تاريخ ووقت",
			month: "شهر",
			monthRange: "نطاق أشهر",
			year: "سنة",
			yearRange: "نطاق سنوات",
			quarter: "ربع سنة",
			quarterRange: "نطاق أرباع السنة",
			week: "أسبوع",
			toggle: "سحب",
			select: "قائمة",
			checkbox: "خانة اختيار",
			radio: "زر اختيار",
			array: "مصفوفة",
			object: "كائن",
			objects: "مجموعة كوائن",
			table: "جدول",
			tableItems: "عناصر من جدول",
			role: "دور",
			mention: "نصوص جاهزة",
			id: "معرف",
			multiple: "متعدد",
			range: "مجال",
			slider: "شريط",
			ip: "عنوان IP",
			json: "JSON",
			custom: "مخصص",
		},

		// Email settings
		emailSettings: "إعدادات البريد الإلكتروني",
		emailConfig: {
			use_custom_smtp: "استخدام SMTP مخصص",
			from_email: "بريد المُرسِل",
			from_name: "اسم المُرسِل",
			smtp_host: "خادم SMTP",
			smtp_port: "منفذ SMTP",
			smtp_user: "اسم مستخدم SMTP",
			smtp_pass: "كلمة مرور SMTP",
			smtp_secure: "اتصال آمن (TLS)",
		},
		smtpPasswordUnchanged: "اتركه فارغًا للاحتفاظ بكلمة المرور الحالية",
		testEmailRecipient: "البريد الإلكتروني لمستلم الاختبار",
		sendTestEmail: "إرسال بريد تجريبي",
		emailSendFailed: "تعذر إرسال البريد الإلكتروني. تحقق من إعدادات البريد.",

		// Email template editor
		emailTemplates: {
			variables: "المتغيرات",
			variableName: "اسم المتغير (مثال: booking_date)",
			add: "إضافة",
			contentPlaceholder:
				"ترميز <mjml>…</mjml> (أو نص عادي) — يتم استبدال المتغيرات مثل {{@user.email}} و {{@data.field}} عند الإرسال.",
			preview: "معاينة ببيانات تجريبية",
			previewLoading: "جارٍ إنشاء المعاينة…",
			previewSubject: "الموضوع",
			previewEmpty: "ابدأ بالكتابة لرؤية المعاينة.",
			recordFieldsHint:
				"جميع حقول السجل الذي يُفعّل البريد متاحة كـ {{field}} أو {{@data.field}}؛ والمستخدم الحالي كـ {{@user.field}}.",
			sendTest: "إرسال تجريبي",
			sendTestHint:
				"يرسل القالب الحالي (حتى لو لم يُحفظ) إلى عنوان البريد أدناه.",
			testRecipient: "بريد المستلم",
		},

		// Database settings
		translationSettings: "إعدادات الترجمة",
		deleteDatabase: "حذف قاعدة البيانات",

		// Tables
		templates: "قوالب البريد الإلكتروني",

		// Flows
		accessDenied: "لا توجد صلاحيات كافية",
		throwError: "خطأ",
		if: "إذا",
		or: "أو",
		newCard: "بطاقة جديدة",
		onRequest: "عند الطلب",
		onResponse: "عند العرض",
		condition: "شرط",
		addSchedule: "إضافة جدولة",
		editSchedule: "تعديل الجدولة",
		cronExpression: "تعبير كرون",
		payloadTemplate: "قالب الحمولة",
		nextRunAt: "التنفيذ القادم",
		lastRunAt: "آخر تنفيذ",
		lastError: "آخر خطأ",
		active: "نشط",
		inactive: "غير نشط",
		preset: "إعداد مسبق",
		excludeDays: "استثناء أيام",
		excludeDaysHelp:
			"لن تعمل الجدولة في الأيام المحددة حتى لو طابقها تعبير كرون.",
		excludedDays: "الأيام المستثناة",
		noExcludedDays: "لا يوجد",
		payloadHelp:
			"استخدم كائناً بصيغة JSON أو كائناً مُحوّلاً إلى نص بواسطة Inison للصف الذي تريد إنشاءه.",
		payloadTemplateVariablesHelp:
			"القوالب: {{ now }}، {{ now + 2h }}، {{ now|iso }}، {{ today|date }}، {{ schedule.id }}، {{ database.slug }}، {{ table.slug }}، {{ run.iso }}.",
		previewResolvedPayload: "معاينة الحمولة بعد الحل",
		resolvedPayloadPreview: "معاينة الحمولة النهائية",
		formatJSON: "تحويل وتنسيق JSON",
		fillExample: "ملء مثال",
		customCronHint: "استخدم تعبير كرون مكوناً من 5 أجزاء بتوقيت UTC.",
		presetDescriptionHourly: "يعمل مرة كل ساعة.",
		presetDescriptionDaily: "يعمل يومياً عند منتصف الليل UTC.",
		presetDescriptionWeekly: "يعمل كل يوم اثنين عند منتصف الليل UTC.",
		presetDescriptionMonthly: "يعمل في أول يوم من كل شهر عند منتصف الليل UTC.",
		presetDescriptionCustom: "استخدم تعبير كرون خاصاً للحصول على جدولة متقدمة.",
		scheduleSummaryNoExcluded: "ستعمل هذه الجدولة في كل الأيام المطابقة.",
		scheduleSummaryWithExcluded: "ستتجاوز هذه الجدولة: {days}.",
		sunday: "الأحد",
		monday: "الاثنين",
		tuesday: "الثلاثاء",
		wednesday: "الأربعاء",
		thursday: "الخميس",
		friday: "الجمعة",
		saturday: "السبت",
		hourly: "كل ساعة",
		daily: "يومي",
		weekly: "أسبوعي",
		monthly: "شهري",
		custom: "مخصص",

		// AI Chatbot
		chatNoChangesApplied: "لم يتم تطبيق أي تغييرات بهذه الإجابة. راجع المقترح وطبّقه لحفظ التغييرات.",
		chatbot: "المساعد الذكي",
		chatWelcomeDefault:
			"مرحباً! يمكنني مساعدتك في إنشاء جداول قاعدة بيانات جديدة أو تعديل الجداول الموجودة. صف ما تحتاجه وسأقوم بإنشاء المخططات لك.",
		chatWelcomeScopedDatabase:
			"مرحباً! يمكنني مساعدتك في إنشاء الجداول وتحديثها داخل قاعدة البيانات الحالية. لإنشاء قاعدة بيانات جديدة، استخدم مساحة قاعدة بيانات Inicontent.",
		chatWelcomeTables:
			"أنا جاهز لمساعدتك في تصميم وإدارة جداول قاعدة البيانات. يمكنك طلب إنشاء جداول جديدة أو تعديل الموجودة.",
		chatWelcomePages:
			"يمكنني مساعدتك في بناء صفحات الموقع. صف الصفحة التي تريدها وسأقوم بإنشاء الهيكل.",
		chatPlaceholder: "صف قاعدة البيانات التي تريد بناءها...",
		chatPlaceholderScopedDatabase:
			"صف الجداول التي تريد إنشاءها أو تحديثها في قاعدة البيانات الحالية...",
		chatPlaceholderShort: "اكتب رسالة...",
		chatPaidPlanRequired: "هذا المساعد الذكي متاح فقط مع خطة مدفوعة نشطة.",
		continue: "متابعة",
		continued: "تمت المتابعة",
		dataPreview: "معاينة البيانات",
		generatingMoreTables: "جارٍ إنشاء الجداول المتبقية...",
		homePage: "الصفحة الرئيسية",
		namesUpdated: "تم تحديث الأسماء",
		newChat: "محادثة جديدة",
		pagePreviewUnavailable:
			"تتوفر معاينة الصفحة الحية في منشئ مواقع Inicontent.",
		applyChanges: "تطبيق التغييرات",
		skipAiHelp: "تخطي مساعدة الذكاء الاصطناعي",
		applied: "تم التطبيق",
		publishData: "نشر البيانات",
		published: "تم النشر",
		translateData: "ترجمة",
		translated: "تمت الترجمة",
		tableDemo: "معاينة الجدول",
		billing: "الفوترة",
		reorderFlows: "إعادة ترتيب التدفقات",
		reorderFields: "إعادة ترتيب الحقول",

		billingUi: {
			title: "الفوترة والاشتراك",
			subtitle: "إدارة الخطة والفواتير وإعدادات الدفع.",
			tabs: {
				subscription: "الاشتراك",
				invoices: "الفواتير",
				settings: "الإعدادات",
			},
			noSubscriptionTitle: "لا يوجد اشتراك نشط",
			noSubscriptionBody: "اختر خطة لتفعيل الفوترة وفتح ميزات التخزين.",
			choosePlanTitle: "اختر خطتك",
			currentPlanTitle: "الخطة الحالية",
			planType: "نوع الخطة",
			planTypes: {
				base: "أساسية",
				enterprise: "مؤسسية",
			},
			status: "الحالة",
			statusLabels: {
				active: "نشط",
				canceled: "ملغي",
				past_due: "متأخر",
				paid: "مدفوع",
				open: "مفتوح",
				void: "ملغى",
				uncollectible: "غير قابل للتحصيل",
				unknown: "غير معروف",
			},
			databaseStorage: "تخزين قاعدة البيانات",
			assetStorage: "تخزين الملفات",
			changePlan: "تغيير الخطة",
			hidePlanPicker: "إخفاء اختيار الخطة",
			cancelSubscription: "إلغاء الاشتراك",
			upgradePlanTitle: "ترقية الخطة أو تغييرها",
			storageUsageTitle: "استخدام التخزين",
			storageLimitExceededTitle: "تجاوز حد التخزين",
			storageLimitExceededBody:
				"لقد تجاوزت حد التخزين. فكر في ترقية خطتك أو حذف البيانات غير المستخدمة.",
			invoicesEmpty: "لا توجد فواتير بعد.",
			paymentSettingsTitle: "إعدادات الدفع",
			paymentMethodTitle: "طريقة الدفع",
			paymentMethodSubtitle: "إدارة طريقة الدفع عبر باي بال",
			managePayment: "إدارة الدفع",
			billingEmailTitle: "البريد الإلكتروني للفوترة",
			updateEmail: "تحديث البريد",
			autoRenewalTitle: "التجديد التلقائي",
			autoRenewalMessage: "{{status}} - سيتم {{action}} في نهاية فترة الفوترة.",
			autoRenew: {
				enabled: "مفعل",
				disabled: "معطل",
				renew: "التجديد تلقائيا",
				notRenew: "عدم التجديد",
				enable: "تفعيل التجديد التلقائي",
				disable: "تعطيل التجديد التلقائي",
			},
			cancelModalTitle: "إلغاء الاشتراك",
			cancelModalBody:
				"هل أنت متأكد من إلغاء الاشتراك؟ ستفقد الوصول إلى الميزات المدفوعة في نهاية فترة الفوترة الحالية.",
			cancelModalKeep: "الاحتفاظ بالاشتراك",
			cancelModalConfirm: "نعم، إلغاء",
			invoiceId: "معرف المعاملة",
			period: "الفترة",
			description: "الوصف",
			date: "التاريخ",
			amount: "المبلغ",
			invoice: "الفاتورة",
			downloadPdf: "تنزيل PDF",
			checkoutSuccess: {
				processingPayment: "جار معالجة الدفع...",
				paymentFailed: "فشل الدفع",
				returnToBilling: "العودة إلى الفوترة",
				subscriptionActivated: "تم تفعيل الاشتراك!",
				thankYou: "شكرا لاختيارك Inicontent.",
				goToDashboard: "الانتقال إلى لوحة التحكم",
				viewBillingDetails: "عرض تفاصيل الفوترة",
				subscriptionStatus: "حالة الاشتراك",
				active: "نشط",
				orderId: "معرف الطلب",
				databaseStorage: "تخزين قاعدة البيانات",
				assetStorage: "تخزين الملفات",
				validUntil: "صالح حتى",
				nextSteps: "الخطوات التالية",
				firstProject: "يمكنك الآن إنشاء أول مشروع لك.",
				confirmationEmail:
					"تم إرسال رسالة تأكيد إلى بريدك الإلكتروني في باي بال. للبدء، توجه إلى لوحة التحكم وأنشئ أول قاعدة بيانات CMS.",
				errors: {
					noPaymentToken:
						"لم يتم العثور على رمز الدفع. يرجى المحاولة مرة أخرى.",
					failedToProcess: "تعذر معالجة الدفع. يرجى التواصل مع الدعم.",
					paymentProcessingFailed: "فشلت معالجة الدفع",
				},
			},
			checkoutCanceled: {
				title: "تم إلغاء الدفع",
				description: "لم يكتمل اشتراكك. لم يتم تحصيل أي رسوم.",
				returnToBilling: "العودة إلى الفوترة",
				backToHome: "العودة إلى الرئيسية",
				supportMessage:
					"إذا واجهت أي مشكلة أثناء الدفع، يرجى التواصل مع فريق الدعم لدينا:",
			},
			credit: {
				balance: "رصيد الحساب",
				balanceSubtitle: "رصيدك المستخدم لتسديد الرسوم الشهرية.",
				addCredit: "إضافة رصيد",
				requestBankTransfer: "طلب تحويل بنكي",
				topupTitle: "شحن رصيد الحساب",
				topupDescription:
					"أضف رصيداً عبر PayPal. سيُستخدم رصيدك تلقائياً لتسديد الرسوم الشهرية.",
				amount: "المبلغ (دولار)",
				amountPlaceholder: "أدخل المبلغ",
				youWillAdd: "ستضيف",
				payWithPayPal: "الدفع عبر PayPal",
				amountTooLow: "الحد الأدنى للمبلغ هو 1$",
				amountTooHigh: "الحد الأقصى للمبلغ هو 1,000$",
				topupFailed: "فشل في بدء الشحن. يرجى المحاولة مرة أخرى.",
				topupSuccess: "تمت إضافة الرصيد بنجاح",
				topupSuccessDescription: "تم تحديث رصيد حسابك.",
				topupCanceled: "تم إلغاء الشحن",
				topupCanceledDescription:
					"تم إلغاء دفعتك عبر PayPal. لم يتم تحصيل أي رسوم.",
				processingTopup: "جاري معالجة دفعتك…",
				orderId: "رقم الطلب",
				amountAdded: "المبلغ المضاف",
				newBalance: "الرصيد الجديد",
				requestTitle: "طلب رصيد بتحويل بنكي",
				requestDescription:
					"أرسل طلب تحويل بنكي. سيقوم المسؤول بالتحقق منه وتطبيق الرصيد على حسابك.",
				note: "مرجع التحويل / ملاحظة",
				notePlaceholder: "مرجع التحويل البنكي أو ملاحظات إضافية",
				noteRequired: "يرجى تقديم مرجع التحويل أو ملاحظة",
				submitRequest: "إرسال الطلب",
				requestSubmitted: "تم إرسال طلب الرصيد. سيراجعه المسؤول قريباً.",
				requestFailed: "فشل في إرسال الطلب. يرجى المحاولة مرة أخرى.",
				pendingRequests: "طلبات الرصيد المعلقة",
				noPendingRequests: "لا توجد طلبات معلقة",
				refreshRequests: "تحديث",
				approve: "موافقة",
				reject: "رفض",
				requestUser: "المستخدم",
				requestAmount: "المبلغ",
				requestNote: "الملاحظة",
				requestStatus: "الحالة",
				graceWarning:
					"اشتراكك في فترة السماح. يرجى إضافة رصيد قبل انتهائها لتجنب التعليق.",
			},
			autoCharge: {
				title: "خصم تلقائي عبر PayPal",
				subtitle: "خصم حساب PayPal تلقائياً عندما يكون الرصيد غير كافٍ.",
				statusEnabled: "مفعّل",
				enable: "تفعيل الخصم التلقائي",
				disable: "تعطيل الخصم التلقائي",
				activating: "جاري تفعيل الخصم التلقائي…",
				activatedTitle: "تم تفعيل الخصم التلقائي",
				activatedDescription:
					"تم حفظ حساب PayPal الخاص بك. سيتم خصم الرسوم الشهرية تلقائياً.",
				activationFailed: "فشل تفعيل الخصم التلقائي",
				missingApprovalSession: "جلسة الموافقة مفقودة. يرجى المحاولة مرة أخرى.",
				setupCanceled: "تم إلغاء الإعداد",
				setupCanceledDescription:
					"لقد ألغيت تفويض PayPal. لم يتم تفعيل الخصم التلقائي.",
			},
		},

		pricingUi: {
			title: "اختر خطتك",
			subtitle: "أسعار بسيطة وواضحة بلا رسوم خفية.",
			databaseStorageLabel: "تخزين قاعدة البيانات (GB)",
			assetStorageLabel: "تخزين الملفات (GB)",
			basePlan: "الخطة الأساسية",
			additionalStorage: "تخزين إضافي",
			totalMonthly: "الإجمالي الشهري",
			subscribeNow: "اشترك الآن",
			upgradePlan: "ترقية الخطة",
			requestQuote: "طلب عرض سعر",
			currentUsage: "استخدام التخزين الحالي",
			usageWarning: "لقد تجاوزت حد التخزين. فكر في ترقية خطتك.",
			faqTitle: "الأسئلة الشائعة",
			faq: {
				changePlanTitle: "هل يمكنني تغيير خطة التخزين في أي وقت؟",
				changePlanBody:
					"نعم. يمكنك ترقية أو خفض خطة التخزين في أي وقت. تسري التغييرات في دورة الفوترة التالية.",
				exceedQuotaTitle: "ماذا يحدث إذا تجاوزت حد التخزين؟",
				exceedQuotaBody:
					"سيتم إشعارك عند الاقتراب من الحد. إذا تجاوزته فسنقوم بفوترة التخزين الإضافي وفق أسعارنا.",
				contractTitle: "هل هناك عقد طويل الأجل؟",
				contractBody:
					"لا. جميع الخطط شهرية بدون التزام طويل. يمكنك الإلغاء في أي وقت.",
				annualDiscountTitle: "هل تقدمون خصومات سنوية؟",
				annualDiscountBody:
					"تواصل مع فريق المبيعات للحصول على أسعار مؤسسية مخصصة وخصومات حجم.",
			},
		},

		pricingPageUi: {
			headerTitle: "خطط الأسعار",
			headerSubtitle:
				"أسعار بسيطة وشفافة بلا رسوم خفية. ادفع فقط مقابل ما تستخدمه.",
			ctaTitle: "جاهز للبدء؟",
			ctaSubtitle:
				"انضم إلى آلاف المطورين الذين يستخدمون Inicontent لإدارة محتواهم.",
			viewSubscription: "عرض اشتراكك",
		},

		// JSON Field
		format: "تنسيق",
		invalidJSON: "JSON غير صحيح",

		// Dashboards
		dashboards: "لوحات المعلومات",
		dashboard: "لوحة المعلومات",
		dashboardName: "اسم لوحة المعلومات",
		dashboardSettings: "إعدادات لوحة المعلومات",
		editDashboard: "تعديل لوحة المعلومات",
		createDashboard: "إنشاء لوحة معلومات",
		dashboardPreview: "معاينة لوحة المعلومات",
		widgets: "الأدوات",
		widget: "أداة",
		addWidget: "إضافة أداة",
		editWidget: "تعديل الأداة",
		widgetTitle: "عنوان الأداة",
		untitledWidget: "أداة بدون عنوان",
		noWidgets: "لا توجد أدوات بعد. أضف واحدة للبدء.",
		widgetCount: {
			zero: "لا توجد أدوات",
			one: "أداة واحدة",
			two: "أداتان",
			few: "{count} أدوات",
			many: "{count} أداة",
			other: "{count} أداة",
		},
		counter: "عداد",
		line: "رسم بياني خطي",
		bar: "رسم بياني شريطي",
		pie: "رسم بياني دائري",
		lineChart: "رسم بياني خطي",
		barChart: "رسم بياني شريطي",
		pieChart: "رسم بياني دائري",
		sourceTable: "الجدول المصدر",
		selectTable: "اختر جدول",
		selectField: "اختر حقل",
		selectType: "اختر نوع",
		operation: "العملية",
		count: "عدد",
		sum: "مجموع",
		max: "أقصى",
		min: "أدنى",
		groupBy: "تجميع حسب",
		dateField: "حقل التاريخ",
		dateRange: "نطاق التاريخ",
		last7Days: "آخر 7 أيام",
		last30Days: "آخر 30 يوم",
		last90Days: "آخر 90 يوم",
		lastYear: "السنة الماضية",
		allTime: "كل الوقت",
		limit: "الحد",
		noData: "لا توجد بيانات",
		columnsToDisplay: "الأعمدة المعروضة",
		sortField: "ترتيب حسب",
		sortOrder: "اتجاه الترتيب",
		asc: "تصاعدي",
		desc: "تنازلي",

		// Translation drawer
		translateItem: "ترجمة العنصر",
		original: "النص الأصلي",
		noSecondaryLanguages: "لا توجد لغات ثانوية مُعدَّة لهذه قاعدة البيانات.",
		noTranslatableFields: "لا توجد حقول نصية قابلة للترجمة في هذا الجدول.",
		translationPlaceholder: "أدخل الترجمة…",
		translationsSaved: "تم حفظ الترجمات",
		translationCount: "{count} ترجمة",

		passkey: {
			securityTitle: "أمان مفتاح المرور",
			description: "سجّل مفتاح مرور لتسجيل الدخول باستخدام التحقق من جهازك.",
			register: "تسجيل مفتاح مرور",
			enrollmentTitle: "إعداد مفتاح مرور",
			enrollmentPrompt:
				"هل ترغب في إعداد مفتاح مرور لتسجيل دخول أسرع وأكثر أمانًا على هذا الجهاز؟",
			notNow: "ليس الآن",
			notSupported: "هذا المتصفح لا يدعم مفاتيح المرور.",
			hint: "يمكنك استخدام Face ID أو Touch ID أو مفتاح أمان بعد التسجيل.",
			registeredSuccessfully: "تم تسجيل مفتاح المرور بنجاح.",
			registrationFailed: "فشل تسجيل مفتاح المرور.",
			signin: "تسجيل الدخول بمفتاح المرور",
			signinHintUsername:
				"أدخل اسم المستخدم أو البريد الإلكتروني ثم تابع باستخدام مفتاح المرور.",
			signinHintDevice:
				"استخدم التحقق من جهازك (Face ID أو Touch ID أو مفتاح أمان).",
			signinFailed: "فشل تسجيل الدخول بمفتاح المرور.",
		},

		// Offline / sync
		offlineReconnect: "عاد الاتصال. زامن التغييرات المحفوظة الآن.",
		offlineBlockedIntro: "هذا التغيير يمنع المزامنة. عدّله وأعد المحاولة أو تجاهله للمتابعة.",
		offlineDiscard: "حذف التغيير المحلي",
		offlineEditRetry: "حفظ وإعادة المحاولة",
		offlineInvalidJson: "أدخل كائن JSON صالحًا.",
		offlineNoSearchColumns: "لا يحتوي الجدول المرجعي على أعمدة بحث. أدخل معرّفًا أو اضبط أعمدة البحث.",
		offlinePickManually: "عُثر على عدة نتائج. اختر يدويًا بإدخال معرّف العنصر الدقيق.",
		offlineReferenceNotFound: "لم يُعثر على عنصر مطابق. أدخل معرّفًا أو صحّح نص البحث.",
		offlineInvalidReference: "تعذّر تحديد المرجع.",
		offlineMissingDependency: "عنصر محلي مرجعي مفقود. استبدل معرّفه المؤقت أو احذف هذا التغيير.",
		offlineDependencyCycle: "تشكّل المراجع المحلية حلقة. عدّل المراجع.",
		offlineMissingCreatedId: "لم يُرجع الخادم معرّف العنصر المنشأ. تحقّق من الخادم قبل إعادة المحاولة.",
		offlineSyncFailed: "فشلت المزامنة. راجع التغيير وأعد المحاولة.",
		online: "متصل",
		offline: "غير متصل",
		onlineTooltip: "متصل بالخادم",
		offlineTooltip:
			"أنت غير متصل. تُحفظ التغييرات محليًا. اضغط المزامنة عند الاتصال.",
		syncingPending: "جارٍ المزامنة…",
		pendingChanges: "{count} تغيير معلق",
		syncNow: "زامن الآن",
		conflictsPending: "{count} تعارض بحاجة إلى حل",
		updateAvailable: "يتوفر تحديث",
		installApp: "تثبيت التطبيق",
		resolveConflicts: "حل التعارضات",
		conflictModalIntro:
			"تغييرك دون اتصال يتعارض مع نسخة أحدث على الخادم. اختر النسخة التي تريد الاحتفاظ بها.",
		noConflicts: "لا توجد تعارضات لحلها",
		discardAll: "تجاهل الكل",
		localVersion: "نسختك",
		serverVersion: "نسخة الخادم",
		keepLocal: "الاحتفاظ بنسختي",
		keepServer: "الاحتفاظ بنسخة الخادم",
		changeQueuedForSync: "تمت إضافة التغيير إلى قائمة المزامنة",
		localChangeDiscarded: "تم تجاهل التغيير المحلي",
		allLocalChangesDiscarded: "تم تجاهل جميع التغييرات المحلية",
		queuedOfflineToast:
			"حُفظ التغيير محليًا. اضغط المزامنة عند الاتصال.",
		createFailed: "تعذر إنشاء العنصر.",
		updateFailed: "تعذر تحديث العنصر.",
		deleteFailed: "تعذر حذف العنصر.",
		errorFetchingSchema: "تعذر جلب مخطط الجدول.",

		// Document scanner
		documentScanner: "ماسح المستندات",
		camera: "الكاميرا",
		uploadImage: "رفع صورة",
		openCamera: "فتح الكاميرا",
		capture: "التقاط",
		retake: "إعادة",
		cameraUnavailable: "الكاميرا غير متاحة",
		loadingScanner: "جارٍ تحميل ماسح المستندات",
		retry: "إعادة المحاولة",
		dropImageOrClick: "أسقط صورة هنا أو انقر للتصفح",
		preview: "معاينة",
		extractingDocument: "جارٍ استخراج المستند",
		ocr: "OCR",
		language: "اللغة",
		extractedText: "النص المستخرج",
		copy: "نسخ",
		insertAsText: "إدراج كنص",
		insertAsImage: "إدراج كصورة",
		useThisScan: "استخدام هذا المسح",
		documentNotDetected: "لم يتم اكتشاف المستند",
		adjustCorners: "ضبط الزوايا",
		dragHandlesToAdjust: "اسحب المقابض الخضراء لتحديد البطاقة ثم طبّق.",
		applyCrop: "تطبيق القص",
		scanDocument: "مسح مستند",

		// مساحات عمل المنصة (الصفحات، الكتل، القوالب، الفوترة)
		openInPlatform: "فتح في إينيكونتينت",
		platformFeatureIntro: "مساحة عمل {{feature}} جزء من منصة إينيكونتينت.",
		platformFeatureAt: "افتحها على:",
		platformFeatureHint: "سيتم تسجيل دخولك تلقائيًا بجلستك الحالية.",

		// النسخ الاحتياطية
		backups: "النسخ الاحتياطية",
		backupsAdminOnly: "فقط مالك القاعدة (المدير) يمكنه إدارة النسخ الاحتياطية.",
		createBackup: "إنشاء نسخة احتياطية",
		backupName: "اسم النسخة",
		backupNamePlaceholder: "مثال: قبل الإطلاق",
		backupNameHelp: "اتركه فارغاً لتوليد اسم تلقائي بالتاريخ والوقت.",
		backupType: "نوع النسخة",
		fullBackup: "القاعدة كاملة",
		tableBackup: "جدول",
		fullBackupHelp:
			"لقطة لمجلد القاعدة بالكامل (الجداول والبيانات الوصفية) كأرشيف خام.",
		tableBackupHelp: "لقطة لمجلد جدول واحد كأرشيف خام.",
		backupCreatedSuccessfully: "بدأت النسخة. ستظهر هنا عند اكتمالها.",
		backupDeletedSuccessfully: "تم حذف النسخة",
		backupOrigin: "المصدر",
		automaticBackup: "تلقائي",
		manualBackup: "يدوي",
		backupSize: "الحجم",
		backupState: "الحالة",
		queued: "في الانتظار",
		running: "قيد التنفيذ",
		completed: "مكتملة",
		failed: "فاشلة",
		restoring: "جارٍ الاستعادة",
		restoreBackup: "استعادة النسخة",
		restoreScope: "نطاق الاستعادة",
		restoreWarning:
			"الاستعادة تستبدل البيانات الموجودة بمحتوى هذه النسخة. لا يمكن التراجع عن هذا الإجراء.",
		restoreConfirmation: "تأكيد",
		restoreConfirmationHelp: "أفهم أن هذا سيستبدل البيانات الموجودة.",
		restoreStarted: "بدأت الاستعادة. سيتم إشعارك عند اكتمالها.",
		restore: "استعادة",
		backupScopeTable: "جدول (المجلد كاملاً)",
		backupScopeTableSchema: "مخطط الجدول فقط",
		backupScopeTableData: "بيانات الجدول فقط",
		backupScopeDatabaseSchema: "مخططات جميع الجداول",
		backupScopeDatabaseData: "بيانات جميع الجداول",
		backupScopeDatabaseFull: "القاعدة كاملة (المخططات + البيانات)",
		folder: "مجلد",
		deleteBackupConfirm:
			'حذف النسخة الاحتياطية "{name}"؟ لا يمكن التراجع عن هذا الإجراء.',
	},
} as const;
