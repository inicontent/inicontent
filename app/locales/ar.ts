export default {
	ar: {
		...
		computedExpressionPlaceholder: "مثال: sum(5, 6) أو 5 , 6 أو 4.2",
		computedExpressionDocs: {
			whatIsIt: "ما هو؟",
			whatIsItBody:
				"يُحتسب الحقل المحسوب تلقائيًا بواسطة المحرك في كل مرة يُحفظ فيها السجل. تُخزَّن النتيجة في السجل وتكون للقراءة فقط — لا يمكن تعديلها يدويًا أو إرسالها عند إنشاء/تحديث السجلات.",
			referencing: "الإشارة إلى الحقول",
			referencingBody:
				"أشِر إلى الحقول الأخرى بمعرفها الرقمي — الظاهر بصيغة #id تحت اسم الحقل. مثال: `2 , 3` يضرب الحقل #2 في الحقل #3.",
			operators: "العوامل",
			operatorsBody:
				"`+` جمع، `-` طرح، `,` ضرب، `/` قسمة، `%` باقي القسمة، أقواس `(...)` للتجميع.",
			functions: "دوال المصفوفات",
			functionsBody:
				"تجميع عناصر حقل من نوع مصفوفة من الكائنات: `sum(id)` المجموع، `count(id)` عدد العناصر، `avg(id)` المتوسط، `min(id)` الحد الأدنى، `max(id)` الحد الأقصى.",
			decimals: "الكسور العشرية",
			decimalsBody:
				"لا توجد أعداد عشرية — `.` محجوز للروابط. استخدم القسمة بدلًا من ذلك: `314 / 100` يعطي 3.14.",
			tables: "الجداول",
			tablesBody:
				"تستطيع الإشارة إلى الحقول الداخلية لجدول مرتبط باستخدام `.`: `3.4` يقرأ الحقل #4 من السجل المرتبط بالحقل #3.",
			notes: "ملاحظات",
			notesBody:
				"لا يمكن أن يكون الحقل محسوبًا ومطلوبًا/فريدًا/بنمط في نفس الوقت — تُمسح هذه عند تعيين تعبير. تُعاد معالجة الحقول المحسوبة بترتيب الاعتماديات، لذا يمكن لحقل محسوب أن يشير إلى حقل محسوب آخر.",
		},
