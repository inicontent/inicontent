<template>
    <NPopover style="max-height: 240px;" :style="`width: ${isMobile ? '350px' : '500px'}`" placement="bottom-end"
        trigger="click" scrollable v-model:show="isSearchPopoverVisible">
        <template #trigger>
            <NTooltip :delay="1500">
                <template #trigger>
                    <NButton round :size="props.size">
                        <template #icon>
                            <NIcon>
...
const searchArray = defineModel<searchType>("array", { default: () => reactive({ and: [[null, "=", null]] }) });
...
async function updateFavoriteFilters(filters: any[]) {
    ...
    try {
        ...
        const updatedUser = { ...user.value, config: { ...(user.value.config ?? {}), filters: allFilters } }
